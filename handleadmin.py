...
@login_required
def admins_dash():
    sum_keys = ['deal_price', 'gross_commission', 'amount_received', 'amount_eligible', 'agent_received', 'agent_pending', 'agent_commission', 'pending_eligible']
...
def fetch_txns():
    ...
    for r in query.filter(Deals.agent_1 == filters_01['agent_1']):
        ...
        new = row2dict(r)
        for k in ['contact_buyer_number','contact_buyer_name','contact_buyer','listing_ref','created_by','type','id','transaction_type','source','priority','deposit','agency_fee_seller','agency_fee_buyer','include_vat','split_with_external_referral','agent_2','commission_agent_2','estimated_deal_date','unit_category','unit_beds','unit_floor','unit_type','buyer_type','finance_type','tenancy_start_date','tenancy_renewal_date','cheques', 'contact_buyer_email','contact_seller','contact_seller_name','contact_seller_number','contact_seller_email', 'status','sub_status','hot_lead','deal_type','bank_representative_name','bank_representative_mobile','referral_date','pre_approval_loan','down_payment_available','down_payment','number_cheque_payment','cheque_payment_type','move_in_date','client_referred_bank','loan_amount','project','floor_no','plot_size','unit_price','percentage','amount']: new.pop(k, None)
...
def recalculatemyg(deal_ref, user, type, amount):
    ...
    if type == 'To company':
        ...
        x = float(deal.amount_received.replace(',', '')) * (float(deal.agent_pers_comm) / 100)
        ...
        x = float(deal.amount_eligible.replace(',', '')) - float(deal.agent_received.replace(',', ''))
